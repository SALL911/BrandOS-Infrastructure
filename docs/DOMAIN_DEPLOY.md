...
### 問題：瀏覽器顯示 `SSL handshake failed` / `Error code 525`

這是 Cloudflare ↔ Vercel origin 之間 TLS 握手失敗。**Browser/Cloudflare 都 Working、只有 Host Error**，表示 Cloudflare 連到 Vercel 那段拿不到有效憑證。

常見三個根因（依機率排序）：
1. **Cloudflare proxy（橘雲）在 Vercel 還沒核發憑證前就打開** — Vercel 的 Let's Encrypt 需要從 `symcio.tw` 直連驗證，橘雲會擋住。
2. **Vercel Domain 狀態退回 `Invalid Configuration`** — 通常是 A record 被改過，或憑證自動續約失敗。
3. **Cloudflare SSL/TLS mode = Full (strict) 但 origin 憑證真的失效** — 兩端同時壞。

**立即救援（1 分鐘讓站點回來）：**
1. Cloudflare → DNS → `symcio.tw` 的 A record **從橘雲改灰雲（DNS-only）**
2. `www` 的 CNAME 同樣改灰雲
3. 等 30 秒，瀏覽器重開 `https://symcio.tw`——流量直達 Vercel，Vercel 自己的憑證會生效

**根因修復順序：**
1. Vercel → Project → Settings → Domains → `symcio.tw` 必須顯示 **Valid Configuration** + **Certificate Issued**
2. 若 Invalid：點 **Refresh**，或移除再重加 domain
3. 憑證 Issued 後，Cloudflare → SSL/TLS → Overview 確認是 **Full (strict)**
4. 再把 Cloudflare DNS 的雲從灰切回橘
5. `curl -I https://symcio.tw` 應回 `200` 且 header 有 `server: Vercel` 或 `cf-ray`

**診斷指令：**
```bash
# 看 origin 憑證（跳過 Cloudflare）
echo | openssl s_client -servername symcio.tw -connect 76.76.21.21:443 2>/dev/null | openssl x509 -noout -subject -issuer -dates

# 看 Cloudflare 回的是什麼
curl -sI https://symcio.tw | grep -iE 'server|cf-ray|cf-cache'

# 確認 DNS 指向
dig symcio.tw A +short
dig www.symcio.tw CNAME +short
```
